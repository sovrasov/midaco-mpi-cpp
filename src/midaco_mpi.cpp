...
MidacoSolution solve_midaco_mpi(const IGOProblem<double>* problem,
                                const MidacoMPIParameters& params,
                                std::function<bool(const double*)> external_stop)
{
  ...
  if (proc == 0)
  {
    ...
    for(int c=0; c<p*params.numThreads; c++)
    {
      std::copy_n(x, n, xxx + c*n);
    }
    ...
    {
        for (int c=2; c<=p; c++) /* Send iterates X for evaluation */
        {
        ...
        /* Collect results F & G */
        MPI_Gather(f, o*params.numThreads, MPI_DOUBLE, fff, o*params.numThreads,
                   MPI_DOUBLE, 0, MPI_COMM_WORLD);
        MPI_Gather(g, m*params.numThreads, MPI_DOUBLE, ggg, m*params.numThreads,
                   MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  else
  {
   ...
   while(istop<=0)
   {
     ...
     MPI_Gather(f, o*params.numThreads, MPI_DOUBLE, nullptr, 0,
                MPI_DOUBLE, 0, MPI_COMM_WORLD);
     MPI_Gather(g, m*params.numThreads, MPI_DOUBLE, nullptr, 0,
                MPI_DOUBLE, 0, MPI_COMM_WORLD);
